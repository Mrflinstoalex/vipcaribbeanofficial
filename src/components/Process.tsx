            ...
            {data.pasos.map((paso, index) => {
              ...
              return (
                <div key={index} className="relative group">
                  <div className="relative p-6 rounded-2xl bg-gradient-card border border-border group-hover:border-primary/30 group-hover:shadow-card transition-all duration-300">
                    {/* Step Number */}
                    <div className="absolute top-2 right-4 text-6xl font-display font-bold text-primary/10 group-hover:text-primary/20 transition-colors select-none">
                      {String(index + 1).padStart(2, "0")}
                    </div>

                    {/* Icon */}
                    {Icon && (
                      <div className="relative w-14 h-14 rounded-xl bg-gradient-coral flex items-center justify-center mb-6 shadow-coral">
